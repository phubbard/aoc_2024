...
impl fmt::Display for LocationID {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        write!(
            ...
            self.value, self.position_number
        )
    }
}
...
fn try_set(problem_set: &ProblemSet) -> Result<(), std::io::Error> {
    ...
    for (line_number, line) in reader.lines().enumerate() {
        ...
        if nums.len() != 2 {
            return Err(io::Error::new(
                io::ErrorKind::InvalidData,
                format!(
                    "Line {} does not contain exactly 2 numbers",
                    line_number + 1
                ),
            ));
        }
    ...
    if problem_set.expected_answer_p2 != accum_p2 {
        return Err(io::Error::new(
            io::ErrorKind::InvalidData,
            format!(
                "Part Deux: Expected {} but saw {}",
                problem_set.expected_answer_p2, accum_p2
            ),
        ));
    }
